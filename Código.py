...
import pandas as pd              # libreria para crear dataframes
import matplotlib.pyplot as plt  # libreria para graficar
import statistics                # libreria para calculos matematicos


# Funcion que permite validar la exitencia del archivo
...
if existe == False:  # si no existe el archivo, sale del programa
    ...
    specialChars = " "
    # bucle para que recorra caracteres y compare con los espacios asi se edita el titulo para su presentacion
    ...
    i = pd.read_csv("temporal.txt", header=0, delim_whitespace=True)
    ...
    if tip == 1:  # Cuando el archivo tiene ambas variables
        ...
        ax1.set_ylabel('Temperatura(C)' + '\n' + 'Promedio: ' + str(avgtemp), color='midnightblue')  # una de las dos variables (Temperatura) en azul
        ax2.set_ylabel('Precipitación(mm)' + '\n' + 'Promedio: ' + str(avgprec), color='maroon')  # una de las dos variables (Precipitacion) en rojo

        line1, = ax1.plot(x, y1, color='dodgerblue',label='Temperatura')  # generamos la combinacion de datos entre el ano y la temperatura
        line2, = ax2.plot(x, y2, color='red',label='Precipitacion')  # generamos la combinacion de datos entre el ano y la precipitacion

        first_legend = plt.legend(handles=[line1], loc='upper right')
        plt.gca().add_artist(first_legend)
        plt.legend(handles=[line2], loc='lower right')

        ax1.axhline(linewidth=4, y=avgtemp, color='midnightblue', linestyle="--")  # muestra linea con promedio
        ax2.axhline(linewidth=4, y=avgprec, color='maroon', linestyle="--")

        fig.tight_layout()  # determina el espacio para la graficacion
        plt.title('Periodo desde ' + str(x[0]) + ' a 2021')  # se coloca como titulo el periodo de tiempo que abarcan los datos
        plt.grid()
        ...
        # inicializamos fig para graficacion de datos
        fig, ax1 = plt.subplots()
        ax1.plot(x, y1, color='dodgerblue', label='Temperatura')  # generamos la combinacion de datos entre el ano y la temperatura
        ax1.axhline(linewidth=4, y=avgtemp, color='midnightblue', linestyle="--")
        ax1.tick_params(axis='y', labelcolor='k')
        ax1.set_xlabel(tit)  # asignamos como label inferior del grafico la variable tit
        ax1.set_ylabel('Temperatura(C)' + '\n' + 'Promedio: ' + str(avgtemp),color='midnightblue')  # una de las dos variables (Temperatura) en azul
        fig.tight_layout()  # determina el espacio para la graficacion
        ...
        ax2.plot(x, y2, color='red', label='Precipitaciones')  # generamos la combinacion de datos entre el ano y la temperatura
        ax2.axhline(linewidth=4, y=avgprec, color='maroon', linestyle="--")
